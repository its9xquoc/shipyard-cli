...
class DeployCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $site = $this->shipyard->site($this->currentServer()->id, $siteId);
    ...
    public function deploy($site)
    {
        ...
        $this->shipyard->deploySite($server->id, $site->id, false);

        $deploymentId = $this->ensureDeploymentHaveStarted($site);

        $deployment = $this->ensureDeploymentHasFinished($server, $site, $deploymentId);

        $output = $this->shipyard->siteDeploymentOutput($server->id, $site->id, $deploymentId);
    ...
    protected function ensureDeploymentHaveStarted($site)
    {
        ...
        $deploymentId = collect($this->shipyard->siteDeployments(
            $this->currentServer()->id,
            $site->id,
        ))->first()['id'];
    ...
    protected function ensureDeploymentHasFinished($server, $site, $deploymentId)
    {
        do {
            ...
            $deployment = $this->shipyard->siteDeployment($server->id, $site->id, $deploymentId);
