...
trait InteractsWithVersions
{
    ...
    /**
     * Warns the user about the latest version of Shipyard CLI.
     *
     * @return void
     */
    protected function ensureLatestVersion()
    {
        ...
        if (version_compare($remote = $this->getLatestVersion(), $current) > 0) {
            $this->warnStep(['You are using an outdated version %s of Shipyard CLI. Please update to %s.', $current, $remote]);
        }
    }
    ...
    protected function getLatestVersion()
    {
        $resolver = static::$latestVersionResolver ?? function () {
            $package = json_decode(file_get_contents(
                'https://packagist.org/p2/laravel/shipyard-cli.json'
            ), true);

            return collect($package['packages']['laravel/shipyard-cli'])
                ->first()['version'];
        };
