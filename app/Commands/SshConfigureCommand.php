...
class SshConfigureCommand extends Command
{
     ...
     * @return string
     */
    protected function ensureKeyExists($name, $key = null, $username = 'shipyard')
    {
        ...
        $this->shipyard->createSSHKey($server->id, ['key' => $key, 'name' => $name, 'username' => $username], true);
    ...
    protected function getServerUsername()
    {
        ...
        return $this->option('user') ?: $this->askStep($question, 'shipyard');
    }
}
