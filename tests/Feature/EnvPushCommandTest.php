...
it('can push environment variables from the generated file', function () {
    ...
    $this->shipyard->shouldReceive('updateSiteEnvironmentFile')->once()->with(1, 2, $content);
    ...
    $this->artisan('env:push')
        ...
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>Would You Like Update The Site Environment File With The Contents Of The File <comment>[.env.shipyard.2]</comment></>', 2)
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>Would You Like To Delete The Environment File <comment>[.env.shipyard.2]</comment> From Your Machine</>', false)
        ->expectsOutput('==> Uploading [.env.shipyard.2] Environment File')
