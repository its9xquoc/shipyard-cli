...
it('can retrieve logs from daemons', function () {
    ...
    $this->client->shouldReceive('daemon')->andReturn(
        (object) ['id' => 1, 'user' => 'shipyard'],
    );

    $this->remote->shouldReceive('tail')
        ->once()
        ->with('/home/shipyard/.shipyard/daemon-1.log', Mockery::type(Closure::class), [])
        ->andReturn([0, [
            '[00:01] FOO',
            '[00:02] BAR',
        ]]);
...
it('can tail logs from daemons', function () {
    ...
    $this->remote->shouldReceive('tail')
        ->once()
        ->with('/home/shipyard/.shipyard/daemon-1.log', Mockery::type(Closure::class), ['-f'])
        ->andReturn([0, [
            '[00:01] FOO',
            '[00:02] BAR',
        ]]);
