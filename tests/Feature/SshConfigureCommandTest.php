...
it('can create ssh keys', function () {
    ...
    $this->shipyard->shouldReceive('server')
        ->once()
        ->with(1)
        ->andReturn((object) [
            'id' => 1,
    ...
    $this->shipyard->shouldReceive('createSSHKey')->with(1, [
        'name' => 'driesvints',
        'key' => 'MY KEY Content',
        'username' => 'morales2k',
    ], true)->once();
...
it('can reuse ssh keys', function () {
    ...
    $this->shipyard->shouldReceive('createSSHKey')->with(2, [
        'name' => 'driesvints',
        'key' => 'MY KEY Content',
        'username' => 'morales2k',
    ], true)->once();
    ...
    $this->artisan('ssh:configure', ['server' => 2])
        ...
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>What Should The SSH Key Be Named In Shipyard</>', 'driesvints')
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>What Username Should We Use For The Selected Server</>', 'morales2k')
