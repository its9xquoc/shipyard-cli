...
it('can open shell connections to mysql databases', function () {
    ...
    $this->client->shouldReceive('databases')->andReturn([
        (object) ['id' => 1, 'name' => 'shipyard-default-database'],
    ]);

    $this->remote->shouldReceive('passthru')
        ->with('mysql -u shipyard -pmy-secret-password shipyard-default-database')
        ->andReturn(0);

    $this->artisan('database:shell')
        ->expectsOutput('==> Establishing Shell Connection To [production@shipyard-default-database] Database')
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>Enter The Database User <comment>[shipyard]</comment> Password</>', 'my-secret-password');
});
...
it('can open shell connections to postgres databases', function () {
    ...
    $this->remote->shouldReceive('passthru')
        ->with('PGPASSWORD=my-secret-password psql -U shipyard shipyard-default-database')
        ->andReturn(0);
...
test('exit code gets returned', function () {
    ...
    $this->remote->shouldReceive('passthru')
        ->with('mysql -u shipyard -pmy-wrong-secret-password shipyard-default-database')
        ->andReturn(1);

    $this->artisan('database:shell')
        ->expectsOutput('==> Establishing Shell Connection To [production@shipyard-default-database] Database')
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>Enter The Database User <comment>[shipyard]</comment> Password</>', 'my-wrong-secret-password')
        ->assertExitCode(1);
});
...
it('can not open shell connections if the database is empty', function () {
    ...
    $this->artisan('database:shell')
        ->expectsOutput('==> Establishing Shell Connection To [production@shipyard-default-database] Database')
        ->expectsQuestion('<fg=yellow>‣</> <options=bold>Enter The Database User <comment>[shipyard]</comment> Password</>', null);
