...
NEW_USER="${NEW_USER:-deploy}"                    # Non-root user
NEW_USER_PASSWORD="${NEW_USER_PASSWORD:-ChangeMe123!}"      # Temporary password for new user
SSH_PORT="${SSH_PORT:-2222}"                      # New SSH port (not 22)
DOMAIN="${DOMAIN:-_}"             # Your actual domain
EMAIL="${EMAIL:-your-email@domain.com}"        # Email for SSL certificate

# Database config
DB_ROOT_PASS="${DB_ROOT_PASS:-ChangeMe123!}"
...
main() {
    ...
    if [[ "${ENABLE_DEPLOY_SCRIPT:-0}" == "1" ]]; then
        create_deploy_script
    fi
